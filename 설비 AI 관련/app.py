...
드래그·리사이즈·추가·삭제가 가능한 자유 배치 그리드 레이아웃
...
from dash import dcc, html, callback, Input, Output, State, no_update, ALL, ctx
...
# ─────────────────────────────────────────────
ALL_PANELS = {
    "panel-explain":    "❓ 건강도와 이상감지란?",
    "panel-gauge":      "📊 설비 건강도",
    "panel-risks":      "⚠️ 주의 센서 TOP 5",
    "panel-timeseries": "📈 시계열 분석",
    "panel-report":     "📋 분석 리포트",
    "panel-priority":   "🗂️ 전체 설비 순위",
}

# 기본 레이아웃 (height=50px 행 기준, gridCols=12)
DEFAULT_LAYOUT = [
    {"i": "panel-explain",    "x": 0, "y": 0,  "w": 12, "h": 4,  "minH": 3, "minW": 3},
    {"i": "panel-gauge",      "x": 0, "y": 4,  "w": 5,  "h": 6,  "minH": 4, "minW": 3},
    {"i": "panel-risks",      "x": 5, "y": 4,  "w": 7,  "h": 6,  "minH": 4, "minW": 3},
    {"i": "panel-timeseries", "x": 0, "y": 10, "w": 12, "h": 9,  "minH": 5, "minW": 3},
    {"i": "panel-report",     "x": 0, "y": 19, "w": 12, "h": 5,  "minH": 3, "minW": 3},
    {"i": "panel-priority",   "x": 0, "y": 24, "w": 12, "h": 7,  "minH": 4, "minW": 3},
]
_DEFAULT_LAYOUT_MAP = {l["i"]: l for l in DEFAULT_LAYOUT}
...
# 패널 콘텐츠 빌더
# ─────────────────────────────────────────────
def _panel_content(panel_id: str) -> html.Div:
    """패널 ID에 맞는 내부 콘텐츠(빈 컨테이너 포함) 반환"""
    if panel_id == "panel-explain":
        return html.Div([
            dbc.Row([
                dbc.Col([
                    html.P("🏥 건강도 점수",
                           style={"color": "#00B4D8", "fontWeight": "bold", "marginBottom": "4px"}),
    ...
    elif panel_id == "panel-gauge":
        return html.Div([
            dcc.Graph(id="gauge-chart", config={"displayModeBar": False}, style={"height": "240px"}),
            html.Div(id="health-status-msg", style={"marginTop": "6px", "fontSize": "0.88em"}),
        ])

    elif panel_id == "panel-risks":
        return html.Div([
            html.P(id="risks-caption",
                   style={"color": "#888", "fontSize": "0.8em", "marginBottom": "8px"}),
            html.Div(id="risks-cards"),
        ])

    elif panel_id == "panel-timeseries":
        return html.Div([
            html.Div(id="category-selector-container"),
            html.P(id="ts-caption",
                   style={"color": "#888", "fontSize": "0.8em", "margin": "4px 0 6px 0"}),
            dcc.Graph(id="timeseries-chart", config={"displayModeBar": True},
                      style={"height": "360px"}),
        ])

    elif panel_id == "panel-report":
        return html.Div([
            html.Pre(id="report-text",
                     style={"fontSize": "0.82em", "color": "#ccc", "whiteSpace": "pre-wrap",
                            "overflowY": "auto", "maxHeight": "200px",
                            "backgroundColor": "#111", "padding": "10px", "borderRadius": "6px"}),
        ])

    elif panel_id == "panel-priority":
        return html.Div([
            ...
            html.P("📊 점수 계산 방법",
                   style={"color": "#00B4D8", "fontSize": "0.85em", "marginBottom": "4px"}),
            html.Ul([
                html.Li("각 센서의 정상 범위를 벗어난 비율만큼 점수 감점 (100점 만점 시작)",
                        style={"fontSize": "0.8em", "color": "#aaa"}),
                html.Li("온도 센서 ×3배 · 전류 ×2.5배 · 전력/압력 ×2배 비중으로 계산",
                        style={"fontSize": "0.8em", "color": "#aaa"}),
                html.Li("이상감지율(%) = 전체 측정 중 정상 범위를 벗어난 값의 비율",
                        style={"fontSize": "0.8em", "color": "#aaa"}),
            ]),
        ])
...
def _make_panel_shell(panel_id: str, layout_entry: dict) -> dash_draggable.DashboardItem:
    """드래그 핸들 헤더 + 콘텐츠를 포함한 DashboardItem 생성"""
    title = ALL_PANELS[panel_id]
    card = html.Div([
        # ── 드래그 핸들 헤더 ──────────────────────────────
        html.Div([
            html.Span("⠿",
            ...
            html.Span(title,
                      style={"color": "#00B4D8", "fontWeight": "bold",
                             "flex": "1", "fontSize": "0.93em"}),
            html.Button(
                "✕",
                id={"type": "close-panel", "index": panel_id},
                n_clicks=0,
                title="패널 숨기기",
                style={
                    "background": "none", "border": "none", "color": "#666",
                    "cursor": "pointer", "fontSize": "1.0em", "padding": "0 6px",
                    "lineHeight": "1",
                },
            ),
        ...
        # ── 콘텐츠 영역 ──────────────────────────────────
        html.Div(
            _panel_content(panel_id),
            className="panel-body",
        ),
    ], className="panel-card", style={"height": "100%", "padding": "0", "overflow": "hidden"})
    ...
    return dash_draggable.DashboardItem(
        ...
        w=layout_entry.get("w", 12),
        h=layout_entry.get("h", 4),
        minH=layout_entry.get("minH", 3),
        minW=layout_entry.get("minW", 2),
    )


def _build_grid_children(visible: list, layout_map: dict) -> list:
    """visible 패널 목록으로 DashboardItem 리스트 생성 (ALL_PANELS 순서 유지)"""
    return [
        _make_panel_shell(pid, layout_map.get(pid, _DEFAULT_LAYOUT_MAP.get(pid, {})))
...
def make_sidebar():
    return html.Div([
        ...
        html.P("📂 데이터 파일",
               style={"color": "#888", "fontSize": "0.85em", "marginBottom": "6px"}),
        dcc.Upload(
            id="upload-data",
            children=html.Div([
        ...
        html.Div(id="upload-status",
                 style={"marginTop": "6px", "fontSize": "0.8em", "color": "#aaa"}),
        ...
        html.P("🎛️ 패널 표시/숨김",
               style={"color": "#888", "fontSize": "0.85em", "marginBottom": "2px"}),
        html.P("패널의 ✕ 버튼으로도 숨길 수 있습니다",
               style={"color": "#555", "fontSize": "0.73em", "marginBottom": "8px"}),
        dbc.Checklist(
            id="panel-visibility-toggle",
            options=[{"label": f" {title}", "value": pid}
                     for pid, title in ALL_PANELS.items()],
            value=list(ALL_PANELS.keys()),
            switch=True,
            style={"color": "#ccc", "fontSize": "0.85em"},
        ),
        ...
        html.Button(
            ...
            style={
                ...
                "color": "#ccc", "width": "100%", "padding": "7px",
        ...
        html.Div(className="divider"),
        html.P("💡 사용법",
               style={"color": "#888", "fontSize": "0.8em", "marginBottom": "4px"}),
        html.Ul([
            html.Li("헤더를 드래그해 이동", style={"fontSize": "0.75em", "color": "#555"}),
            html.Li("모서리를 드래그해 크기 조절", style={"fontSize": "0.75em", "color": "#555"}),
            html.Li("✕ 버튼으로 숨기기", style={"fontSize": "0.75em", "color": "#555"}),
            html.Li("위 토글로 다시 표시", style={"fontSize": "0.75em", "color": "#555"}),
...
app.layout = html.Div([
    ...
    dcc.Store(id="layout-store", data=DEFAULT_LAYOUT),
        ...
        dbc.Row([
            ...
            dbc.Col([
                html.Div([
                    ...
                    html.Hr(style={"borderColor": "#2a2a4a", "margin": "8px 0 12px 0"}),
                ]),

                dash_draggable.GridLayout(
                    id="main-grid",
                    children=_build_grid_children(
                        list(ALL_PANELS.keys()), _DEFAULT_LAYOUT_MAP
                    ),
                    ...
                    isDraggable=True,
                    isResizable=True,
                    save=False,     # 레이아웃을 서버(layout-store)에서 관리
                ),
...
def process_upload(contents, filename):
    ...
    try:
        ...
        _, _, df_raw = load_raw(io.BytesIO(decoded), file_ext)
        df_clean = clean_numeric(df_raw)
        equip_dict = extract_equipment(df_clean)

        if not equip_dict:
            return None, html.Span("⚠️ 분석 가능한 설비 없음",
                                   style={"color": "#E74C3C"}), [], None

        equip_json = {k: v.to_json(date_format="iso") for k, v in equip_dict.items()}
        options = [{"label": n, "value": n} for n in equip_json]
        status = html.Span(f"✅ {filename}", style={"color": "#2ECC71"})
        return equip_json, status, options, options[0]["value"]

    except Exception as e:
        return None, html.Span(f"❌ 오류: {str(e)[:80]}",
                               style={"color": "#E74C3C"}), [], None


# ─────────────────────────────────────────────
# 콜백 2: 설비/윈도우/스토어 변경 → 모든 패널 콘텐츠 업데이트
# ─────────────────────────────────────────────
@callback(
...
def update_all_panels(selected, window, equip_json_dict):
    empty_fig = _empty_figure("파일을 업로드하고 설비를 선택하세요")
    ...
    try:
        ...
        health_msg = html.Div([
            html.Span(f"■ {level}",
                      style={"color": bar_color, "fontWeight": "bold", "fontSize": "1.0em"}),
        ...
        ts_caption = f"빨간 음영 = 이상 시간대  |  굵은 선 = {window}분 이동 평균  |  흐린 선 = 원시값"
        ts_fig = _make_timeseries(df, anomaly_df, selected_cols[:10], rolling_df, window)
        ...
        return _empty_figure(), err, "", [], "", "", _empty_figure(), str(e), "", []


# ─────────────────────────────────────────────
# 콜백 3: 카테고리 선택 변경 → 시계열 차트 업데이트
# ─────────────────────────────────────────────
@callback(
...
# ─────────────────────────────────────────────
# 콜백 4: 패널 ✕ 버튼 → 사이드바 체크리스트에서 제거
# ─────────────────────────────────────────────
@callback(
    Output("panel-visibility-toggle", "value"),
    Input({"type": "close-panel", "index": ALL}, "n_clicks"),
    State("panel-visibility-toggle", "value"),
    prevent_initial_call=True,
)
def close_panel_via_x(n_clicks_list, current_visible):
    if not ctx.triggered_id or not isinstance(ctx.triggered_id, dict):
        return no_update
    if not any(n for n in (n_clicks_list or []) if n):
        return no_update
    panel_id = ctx.triggered_id["index"]
    return [p for p in (current_visible or []) if p != panel_id]


# ─────────────────────────────────────────────
# 콜백 5: 사이드바 체크리스트 변경 → 그리드 재구성
# ─────────────────────────────────────────────
@callback(
    Output("main-grid", "children"),
    Input("panel-visibility-toggle", "value"),
    State("layout-store", "data"),
)
def update_grid_structure(visible_panels, stored_layout):
    visible = visible_panels or []
    layout_map = {l["i"]: l for l in (stored_layout or DEFAULT_LAYOUT)}
    return _build_grid_children(visible, layout_map)


# ─────────────────────────────────────────────
# 콜백 6: 드래그·리사이즈 후 레이아웃 서버 저장
# ─────────────────────────────────────────────
@callback(
...
def save_layout(current_layout):
...
@callback(
    ...
    Output("panel-visibility-toggle", "value", allow_duplicate=True),
...
def reset_layout(n_clicks):
    ...
    return DEFAULT_LAYOUT, list(ALL_PANELS.keys())
...
def _make_risk_cards(top_risks: list, total_rows: int) -> list:
    ...
    for i, risk in enumerate(top_risks, 1):
        ...
        if "normal_min" in risk:
            range_info = [
                html.Span(f"정상범위: {risk['normal_min']} ~ {risk['normal_max']}",
                          className="risk-range"),
                html.Span(f"평균: {risk.get('avg_value', '-')}  최대: {risk.get('actual_max', '-')}",
                          className="risk-range"),
            ]
        ...
        cards.append(html.Div([
            html.Div([
                ...
                html.Span(f"  [{risk['category']}]",
                          style={"fontSize": "0.78em", "color": "#888"}),
            ], style={"marginBottom": "4px"}),
            html.Div([
                html.Span("건강도: "),
                html.Span(f"{score:.1f}점",
                          style={"color": score_color, "fontWeight": "bold"}),
                html.Span("  |  이상 감지: "),
                html.Span(f"{risk['anomaly_count']}건 / {total}건 ({anom_pct:.1f}%)",
...
def _make_timeseries(df, anomaly_df, selected_cols, rolling_df, window) -> go.Figure:
    ...
    if not selected_cols:
        return _empty_figure("센서 카테고리를 선택하세요")
    ...
    for ts, val in combined_anomaly.items():
        if val and not in_anom:
            ...
            fig.add_vrect(x0=start_t, x1=ts,
...
def _make_priority(equip_json_dict: dict):
    ...
    if all_good:
        ...
        parts = []
        if critical_cnt:
            parts.append(f"🔴 즉시 점검 필요: {critical_cnt}개 설비")
        if warn_cnt:
            parts.append(f"🟡 주의 관찰: {warn_cnt}개 설비")
    ...
    for _, row in priority_df.iterrows():
        ...
        rows.append(html.Tr([
            html.Td(str(int(row["점검순서"])),
                    style={"textAlign": "center", "width": "60px", "color": "#888"}),
            html.Td(row["설비명"], style={"fontWeight": "bold", "color": "#fff"}),
            html.Td(f"{score:.1f}점",
                    style={"fontWeight": "bold", "color": fg,
    ...
    table = dbc.Table(
        ...
        color="dark", size="sm", style={"fontSize": "0.83em"},
    )
